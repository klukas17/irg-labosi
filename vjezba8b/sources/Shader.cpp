...
Shader::Shader(const char * vertexPath, const char * fragmentPath, const char * geometryPath, bool geom)
{
...
Shader* Shader::loadShader(char* path, char* naziv, bool geom) {
    ...
    return new Shader(pathVert.c_str(), pathFrag.c_str(), pathGeom.c_str(), geom);
}