...
int main(int argc, char* argv[]) {
    GLFWwindow* window;
    glfwInit();
    window = glfwCreateWindow(width, height, "Zadatak 10", nullptr, nullptr);
    if (window == nullptr) {
    ...
    if (scene->HasMeshes() && scene->HasMaterials()) {
        ...
        Light* light = new Light(glm::vec3(-1.5f, 7.0f, 14.0f), glm::vec3(0.05, 0.05, 0.05), glm::vec3(0.9, 0.9, 0.9), glm::vec3(1, 1, 1));
        renderer->light = light;

        ShadowMap* shadow_map = new ShadowMap();
        ...
        for (int i = 0; i < scene->mNumMaterials; i++) {
            ...
            if (AI_SUCCESS == scene->mMaterials[i]->Get(AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, 0), texturePosition)) {
                ...
                // ! ne bi trebalo ovako al drukčije ne radi
                texPath.append("/glava/textures/_Diffuse.png");
        ...
        for (int i = 0; i < m->mNumVertices; i++)
            uv.push_back({m->mTextureCoords[0][i].x, m->mTextureCoords[0][i].y});
        ...
        Shader* shader_map = Shader::loadShader(argv[0], "shader_shadow", false);
        for (int i = 0; i < 10; i++)
            for (int j = 0; j < 10; j++) {
                Mesh* mesh = new Mesh(v, n, uv, f, shader, shader_map);
                Transform* transform = new Transform(mesh);
                Object* object = new Object(mesh, transform, material, texture);
        ...
        for (int i = 0; i < 10; i++) {
            z = -11;
            for (int j = 0; j < 10; j++) {
                renderer->objects[index]->transform->setPosition(glm::vec3(x, 0, z));
                z += 2.2;
                index++;
            }
            x += 2.2;
    ...
    while (glfwWindowShouldClose(window) == false) {
        ...
        glViewport(0, 0, renderer->shadow_map->SHADOW_WIDTH, renderer->shadow_map->SHADOW_HEIGHT);
        glBindFramebuffer(GL_FRAMEBUFFER, renderer->shadow_map->depthMapFBO);
